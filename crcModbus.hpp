...
{
    private:
        static uint16_t crc_table16[256];
...
uint16_t crcModbus::crc_modbus(const unsigned char *input_str, size_t num_bytes)
{
    uint16_t crc;
    ...
    {
        crc = (crc>>8) ^ crcModbus::crc_table16 [(crc ^ (uint16_t)*ptr++) & 0x00FF];
    }
    return crc;
}

uint16_t crcModbus::crc_table16[256] = {
